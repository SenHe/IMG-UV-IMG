...
def resample_uv_tensors_to_bbox(
    ...
    canvas_v[:,:,y:y+h,x:x+w] = v_bbox # new_line
    uv = torch.zeros([2, im_size[1], im_size[0]], dtype=torch.float32, device=u.device)
    for part_id in range(1, canvas_u.size(1)):
        uv[0][labels == part_id] = canvas_u[0, part_id][labels == part_id]
        uv[1][labels == part_id] = canvas_v[0, part_id][labels == part_id]
    return uv
...
def densepose_chart_predictor_output_to_result_with_confidences(
    ...
    labels = resample_fine_and_coarse_segm_to_bbox(predictor_output, box_xywh, im_size).squeeze(0) # changed line
